...
na\"{i}ve substitution can result in final expressions for deeply nested
recursive functions becoming too large for Z3 to handle\footnote{In theory, we
could merge common parts of expressions to reduce the number of nodes
generated. In our experiments, this proved to be ineffective for most non-trivial
expressions.}
...
lists the minimal set of changes that need to be made to port
existing SMT-LIB code to \texttt{z3.rkt}. We expect that many SMT-LIB programs
will become shorter as authors use Racket features wherever appropriate.
