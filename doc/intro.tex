...
9$\times$9 grid with the numbers 1 through 9 in such a way that no row, column,
or 3$\times$3 box has two instances of a number. This is a classic constraint
satisfaction problem that any constraint solver can handle with ease.
...
(define (solve-sudoku grid)
  (smt:with-context
   ...
   (if (eq? sat 'sat)
       ;; Retrieve all the values and return them in a list
       (for/list ([x (in-range 0 81)])
         (smt:eval (select sudoku-grid ,x)))
       #f)))
\end{verbatim}

Here \texttt{add-sudoku-grid-rules} asserts the standard Sudoku grid rules, and
\texttt{add-grid} reads the grid in a particular format and creates assertions
based on the grid. We note that the SMT-LIB function \texttt{(select arr x)}
retrieves the value at \texttt{x} from the array \texttt{arr}, and that this can
be used to add constraints on the array (for instance, \texttt{(assert (=
  (select arr x) y))}).
